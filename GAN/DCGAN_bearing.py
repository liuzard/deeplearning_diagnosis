...
def get_optimizer(g_loss, d_loss, beta1=0.4, learning_rate=0.001):
    ...
    with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS)):
        g_opt = tf.train.AdamOptimizer(0.0002, beta1=beta1).minimize(g_loss, var_list=g_vars)
        d_opt = tf.train.AdamOptimizer(0.0005, beta1=beta1).minimize(d_loss, var_list=d_vars)
...
def plot_images(samples):
    ...
    for img, ax in zip(samples, axes):
        print(img.shape)
        ax.plot(x,np.squeeze(img.reshape(1,1024),0))
...
def train(noise_size, data_shape, batch_size, n_samples):
    ...
    inputs_real, inputs_noise = get_inputs(noise_size, data_shape[1], data_shape[2], data_shape[3])#noise_size=100

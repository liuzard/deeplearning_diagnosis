...
from LeNet_1d_diagnosis import LeNet_1d_train
    ...
    with tf.Graph().as_default() as g:
        ...
        x = tf.placeholder(tf.float32, [
            ...
            LeNet_1d_inference.IMAGE_CHANNELS],
                           name='x-input')
        y_=tf.placeholder(dtype=tf.float32, shape=(None, LeNet_1d_inference.IMAGE_LABELS), name="y-input")
        hidden,y= LeNet_1d_inference.inference(x, train=False, regularizer=None)
        # validation_feed={x:mnist.validation.images,y_:mnist.validation.labels}
        correct_predict=tf.equal(tf.argmax(y,1),tf.argmax(y_,1))
        accuracy=tf.reduce_mean(tf.cast(correct_predict,dtype=tf.float32))
        variable_averages=tf.train.ExponentialMovingAverage(LeNet_1d_train.MOVING_AVERAGE_DECAY)
        ...
        while True:
            with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
                ...
                reshaped_xs = np.reshape(xs, (
                    ...
                    LeNet_1d_inference.IMAGE_WIDTH,
                    LeNet_1d_inference.IMAGE_CHANNELS))
                ckpt=tf.train.get_checkpoint_state(LeNet_1d_train.SAVE_PATH)
...
def main(argv=None):
    ...
    test_samples, test_labels,data_tag = LeNet_1d_train.data_preprocess(test_data)

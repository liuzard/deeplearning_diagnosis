...
def inference(input_tensor,train,regularizer):
    ...
    with tf.variable_scope("layer4-conv2"):
        ...
        conv2 = tf.nn.conv1d(pool1, filter, stride=3, padding='SAME')
    ...
    with tf.variable_scope("layer7-conv3"):
        ...
        conv3 = tf.nn.conv1d(pool2, filter, stride=3, padding='SAME')
    ...
    with tf.variable_scope('layer8-conv4'):
        ...
        conv4 = tf.nn.conv1d(relu3, filter, stride=3, padding='SAME')
    ...
    with tf.variable_scope("layer9-conv5"):
        ...
        conv5 = tf.nn.conv1d(relu4, filter, stride=3, padding='SAME')
    ...
    with tf.variable_scope("layer11-fc1"):
        fc1_wieghts=tf.get_variable("weight",[nodes,FC1_WIDTH],initializer=tf.truncated_normal_initializer(stddev=0.1))
    ...
    with tf.variable_scope('layer12-fc2'):
        fc2_weights=tf.get_variable("weights",[FC1_WIDTH,FC2_WIDTH],initializer=tf.truncated_normal_initializer(stddev=0.1))
    ...
    with tf.variable_scope('layer13-fc3'):
        fc3_weights=tf.get_variable("weight",[FC2_WIDTH,FC3_WIDTH],initializer=tf.truncated_normal_initializer(stddev=0.1))

...
data_excel_path = r'C:\Users\liuzard\PycharmProjects\deep_learning\DCAE_diagnosis\experiment1.xlsx'
...
def plot_multomodel_comparasion(ACNN_acc, SDAE_acc,SDAE_denoise_acc, bp_acc, svm_acc):
    x = range(-2, 13, 1)
    plt.plot(x, ACNN_acc, 'b-D', label='DCAE-1D + ACNNDM-1D', markersize=10, linewidth=4)
    plt.plot(x, SDAE_acc, 'r-s', label='SDAE', markersize=10, linewidth=4)
    plt.plot(x, SDAE_denoise_acc, 'k->', label='DCAE-1D + SDAE', markersize=10, linewidth=4)
    plt.plot(x, bp_acc, 'g-o', label='DCAE-1D + BP', markersize=10, linewidth=4)
    plt.plot(x, svm_acc, 'c-X', label='DCAE-1D + SVM', markersize=14, linewidth=4)
...
def plot_reconstruct_error(dcae_error, dae_error):
    x = range(-2, 13, 1)
    plt.plot(x, dcae_error, 'b-D', label='DCAE-1D', markersize=8, linewidth=4)
    plt.plot(x, dae_error, 'r-s', label='DAE', markersize=8, linewidth=4)
    # plt.gca().spines['top'].set_visible(False)  # 去掉上边框
    # plt.gca().spines['right'].set_visible(False)  # 去掉右边框
    plt.tick_params(length=10)
    ...
    plt.xticks(np.arange(-2, 13, 1), fontsize=30)
    plt.yticks(np.arange(0, 0.021, 0.005), fontsize=30)
    plt.grid(axis='both')


def plot_dcae_dae_none_compara(dcae_acc, dae_acc, non_acc):
    x = range(-2, 13, 1)
    plt.plot(x, dcae_acc, 'b-D', label='DCAE-1D + AICNN-1D', markersize=10, linewidth=4)
    plt.plot(x, dae_acc, 'r-s', label='DAE + AICNN-1D', markersize=10, linewidth=4)
    plt.plot(x, non_acc, 'g-o', label='AICNN-1D without denoise', markersize=10, linewidth=4)
...
def plot_drop_pooling_val(full_acc,without_corrupt_acc,without_pooling_acc):
    x = range(-2, 13, 1)
    plt.plot(x, full_acc, 'b-D', label='DCAE-1D + AICNN-1D', markersize=10, linewidth=4)
    plt.plot(x, without_corrupt_acc, 'r-s', label='DCAE-1D + AICNN-1D（clean input training）', markersize=10, linewidth=4)
    plt.plot(x, without_pooling_acc, 'g-o', label='DCAE-1D + AICNN-1D（fully-connected）', markersize=10, linewidth=4)
    # plt.plot(x, svm_acc, 'y-X', label='SVM', markersize=8, linewidth=2)
    # plt.gca().spines['top'].set_visible(False)  # 去掉上边框
    # plt.gca().spines['right'].set_visible(False)  # 去掉右边框
    plt.tick_params(length=10)
...
gear_dcae_error = data_testbed[:, 24]
gear_dae_error = data_testbed[:, 25]
...
gear_dcae_acc = data_testbed[:, 26]
gear_dae_acc = data_testbed[:, 27]
gear_none_acc = data_testbed[:, 28]
...
bp_acc=data_testbed[:,15]
svm_acc=data_testbed[:,16]
...
# 不同模型的对比曲线图
plot_multomodel_comparasion(AICNN_acc, SDAE_acc, SDAE_denoise_acc,bp_acc,svm_acc)

...
class AdditiveGaussianNoiseAutoencoder(object):
    def __init__(self, n_input, n_hidden, transfer_function = tf.nn.tanh, optimizer = tf.train.GradientDescentOptimizer(learning_rate=0.001),
                 scale = 0.1):
        ...
        # cost
        self.cost = 0.5 * tf.reduce_mean(tf.pow(tf.subtract(self.reconstruction, self.x), 2.0))+tf.add_n(tf.get_collection('losses'))

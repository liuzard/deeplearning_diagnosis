...
from DAE_diagnosis import  DAE_diagnosis_train
    ...
    with tf.Graph().as_default() as g:
        ...
        x = tf.placeholder(tf.float32, [
            ...
            DAE_diagnosis_inference.IMAGE_CHANNELS],
                           name='x-input')
        y_=tf.placeholder(dtype=tf.float32, shape=(None, DAE_diagnosis_inference.IMAGE_LABELS), name="y-input")
        y= DAE_diagnosis_inference.inference(x, train=False, regularizer=None)
        validation_feed={x:mnist.validation.images,y_:mnist.validation.labels}
        correct_predict=tf.equal(tf.argmax(y,1),tf.argmax(y_,1))
        accuracy=tf.reduce_mean(tf.cast(correct_predict,dtype=tf.float32))
        variable_averages=tf.train.ExponentialMovingAverage(DAE_diagnosis_train.MOVING_AVERAGE_DECAY)
        ...
        while True:
            with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
                ...
                reshaped_xs = np.reshape(xs, (
                    ...
                    DAE_diagnosis_inference.IMAGE_WIDTH,
                    DAE_diagnosis_inference.IMAGE_CHANNELS))
                ckpt=tf.train.get_checkpoint_state(DAE_diagnosis_train.SAVE_PATH)

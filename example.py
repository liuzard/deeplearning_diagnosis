...
# 处理好之后的数据文件。
INPUT_DATA = 'E:/flower_processed_data.npy'
# 保存训练好的模型的路径。
TRAIN_FILE = 'E:/inception_v3_model'
# 谷歌提供的训练好的模型文件地址。因为GitHub无法保存大于100M的文件，所以
# 在运行时需要先自行从Google下载inception_v3.ckpt文件。
CKPT_FILE = 'E:\inception_v3.ckpt'
...
def main():
    ...
    reshaped_train_images = np.reshape(training_images, (num_train,28,28,1))
    n_training_example = len(training_images)
    training_labels = mnist.train.labels

    num_validation = mnist.validation.num_examples
    validation_images = mnist.validation.images
    reshaped_validation_images = np.reshape(validation_images, (num_validation, 28, 28, 1))
    validation_labels = mnist.validation.labels

    num_test = mnist.test.num_examples
    testing_images = mnist.test.images
    reshaped_test_images = np.reshape(testing_images, (num_test, 28, 28, 1))
    testing_labels = mnist.test.labels
    print("%d training examples, %d validation examples and %d testing examples." % (
        n_training_example, len(validation_labels), len(testing_labels)))

    # 定义inception-v3的输入，images为输入图片，labels为每一张图片对应的标签。
    images = tf.placeholder(tf.float32, [None, 32, 32, 1], name='input_images')
    ...
    with tf.Session() as sess:
        ...
        for i in range(STEPS):
            _, loss = sess.run([train_step, total_loss], feed_dict={
                images: reshaped_train_images[start:end],
            ...
            if i % 30 == 0 or i + 1 == STEPS:
                ...
                validation_accuracy = sess.run(evaluation_step, feed_dict={
                    images: reshaped_validation_images, labels: validation_labels})
        ...
        test_accuracy = sess.run(evaluation_step, feed_dict={
            images: reshaped_test_images, labels: testing_labels})

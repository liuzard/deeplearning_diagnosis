...
# #the fouth convolutional layer para
# CONV4_WIDTH=3
# CONV4_DEPTH=1
...
with tf.variable_scope("layer1-conv1"):
    ...
    print(conv1_biases)
    conv1 = tf.nn.conv1d(inputs_, filter1, stride=2, padding='SAME')
...
with tf.variable_scope("layer2-conv2"):
    ...
    conv2 = tf.nn.conv1d(relu1, filter2, stride=2, padding='SAME')
...
with tf.variable_scope("layer3-conv3"):
    ...
    conv3 = tf.nn.conv1d(relu2, filter3, stride=2, padding='SAME')
...
with tf.variable_scope("layer4-deconv1"):
    ...
    deconv1 = tf.contrib.nn.conv1d_transpose(relu3, filter4, output_shape=[batch_size, 500, DECONV1_DEPTH], stride=3,
                                             padding="SAME")
    relu4 = tf.nn.tanh(tf.nn.bias_add(deconv1, deconv1_biases))
...
with tf.variable_scope("layer5-deconv2"):
    ...
    deconv2 = tf.contrib.nn.conv1d_transpose(relu4, filter5, output_shape=[batch_size, 1000, DECONV2_DEPTH], stride=3,
                                             padding="SAME")
    relu5 = tf.nn.tanh(tf.nn.bias_add(deconv2, deconv2_biases))
...
with tf.variable_scope("layer6-deconv3"):
    ...
    deconv3 = tf.contrib.nn.conv1d_transpose(relu5, filter6, output_shape=[batch_size, 1024, DECONV3_DEPTH], stride=3,
                                             padding="SAME")
    # layer2 = tf.layers.conv2d_transpose(layer1, 256, 4, strides=2, padding='SAME')
    outputs_ = tf.nn.bias_add(deconv3, deconv3_biases)

# with tf.variable_scope("layer7_conv4"):
#     filter7 = tf.get_variable("filter7", [CONV4_WIDTH, DECONV3_DEPTH, CONV4_DEPTH],
#                               initializer=tf.truncated_normal_initializer(stddev=0.1), dtype=tf.float32)
#
#     conv4_biases = tf.get_variable("bias", [CONV4_DEPTH], initializer=tf.constant_initializer(0.0))
#     conv4 = tf.nn.conv1d(relu6, filter7, stride=1, padding='SAME')
#     outputs_ = tf.nn.tanh(tf.nn.bias_add(conv4, conv4_biases))

print(outputs_)
loss = 0.5 * tf.pow(tf.subtract(outputs_, targets_), 2)
